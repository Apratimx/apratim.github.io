...
function resetCanvas() {
    ...
    w = width / cols;
    h = height / rows;
    grid = twoDArray(rows, cols);
    startButton = document.getElementById("startButton")
    startButton.disabled = false
    startButton.innerHTML = "Visualize"
    startButton.onclick = start; //initialises the 
    ...
    // otherwise Reinitializing old source & destination from grid's new objects
    else {
        grid.forEach(row => {
            row.forEach((node) => {
    ...
    destination.show(color(255,255,0));
    noLoop();
    console.log(openSet)
}
...
function dijkstraInitialize() {
    ...
    // Creating a openSet initializing with all the node of the grid
    grid.forEach(row => {
        row.forEach(node => {
            openSet.push(node)
        })
...
function dropdown(event) {
    ...
    if (algo === "A* Search") {
        ...
        message.innerHTML = `Insight: Breadth First Search (BFS) <span style = "font-weight: bold;">Gurantees</span> Shortest Path In An <span style = "font-weight: bold;">Unweighted grid</span> And A Feasible Choice <span style = "font-weight: bold;">If The Destination Is Closer To The Source</span>`
    }
...
function throwObstacles() {
    // It maintains obstacle's distribution in the grid
    let weights = [
        ["Obstacle", 30],
        ["Non Obstacle", 70]
    ]
    ...
    for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
            if (grid[i][j] != source && grid[i][j] != destination) {
...
function mouseDragged() {
    ...
    for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
            ...
            if (mouseX >= grid[i][j].x && mouseX <= grid[i][j].x + grid[i][j].r && mouseY >= grid[i][j].y && mouseY <= grid[i][j].y + grid[i][j].r) {
                console.log("in IF");
                if (grid[i][j] != source && grid[i][j] != destination) {
                    grid[i][j].clicked();
                }
